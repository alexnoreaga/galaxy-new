...
export function TrustBar() {
  return (
    <div className="px-2 sm:px-0 py-3 sm:py-4">
      <div className="grid grid-cols-4 gap-1.5 sm:gap-3">
        {items.map((item) => (
          <div
            key={item.label}
            className="flex flex-col sm:flex-row items-center sm:items-start gap-1 sm:gap-3 bg-white rounded-xl border border-gray-100 shadow-sm px-2 py-3 sm:px-4 sm:py-3.5"
          >
            <div className={`${item.bg} ${item.color} rounded-lg p-1.5 sm:p-2 flex-shrink-0`}>
              {item.icon}
            </div>
            <div className="text-center sm:text-left min-w-0">
              <p className="text-[10px] sm:text-xs font-bold text-gray-900 leading-tight">{item.label}</p>
              <p className="text-[9px] sm:text-[11px] text-gray-400 leading-tight hidden sm:block">{item.sub}</p>
            </div>
          </div>
        ))}
      </div>
